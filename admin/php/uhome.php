...
if($pdone=="error"){
	...
	if($data==0)
	{
		$q1="INSERT INTO `personal_setup`(`profilepic`, `name`, `twitter`, `facebook`, `instagram`, `skype`, `linkedin`, `github`, `homewallpaper`, `professions`, `location`, `mobile`, `emailid`, `faculty_id`) VALUES ('$profilepic','$name','$twitter','$facebook','$instagram','$skype','$linkedin','$github','$homewallpaper','$profession','$address','$mobile','$email','$_SESSION[id]')";
		echo $q1;    
$queryrun=mysqli_query($db,$q1);
if($queryrun){
    header("location:../?edithome=true&msg=Inserted");
	}
}
	else
	{
...
$query.="mobile='$mobile',";
$query.="emailid='$email' WHERE id='$data[id]'";
echo $query;    
$queryrun=mysqli_query($db,$query);
if($queryrun){
